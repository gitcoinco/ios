...
//    let disposeBag = DisposeBag()
    ...
    var profileSignedInViewController: ProfileSignedInViewController?
    var profileSignedOutViewController: ProfileSignedOutViewController?
...
//        observeUserActions()
    ...
    // Reference the 2 children viewcontrollers
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let destination = segue.destination
        if let signedInController = destination as? ProfileSignedOutViewController {
            profileSignedOutViewController = signedInController
        }
        
        if let signedOutController = destination as? ProfileSignedOutViewController {
            profileSignedOutViewController = signedOutController
        }
    }
